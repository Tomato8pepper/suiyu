...
namespace 通讯录管理系统1
{
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connstr))
            {
                try
                {
                    ...
                    SqlParameter p = new SqlParameter("@id", textBox2.Text);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string connstr = @"Data Source=(LocalDB)\v11.0;AttachDbFilename=D:\BIANXHENG\Visual Studio 2012\通讯录管理系统\通讯录管理系统\TelDB.mdf;Integrated Security=True";
                string sqlstr = string.Format("insert into T_User values({0},{1})", textBox2.Text, textBox1.Text);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connstr))
            {
                try
                {
                    ...
                    cmd.Parameters.Add(p);
                    cmd.Parameters.Add("@pwd", SqlDbType.NVarChar).Value = textBox5.Text;
                    conn.Open();
                    if ((int)cmd.ExecuteScalar() > 0)
                    {
                        MessageBox.Show("登陆成功");
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            {
                string connstr = @"Data Source=(LocalDB)\v11.0;AttachDbFilename=D:\BIANXHENG\Visual Studio 2012\通讯录管理系统\通讯录管理系统\TelDB.mdf;Integrated Security=True";
                string sqlstr = string.Format("insert into T_Manage values({0},{1})", textBox3.Text, textBox5.Text);
                SqlConnection conn = new SqlConnection(connstr);
                SqlCommand cmd = new SqlCommand(sqlstr, conn);
                conn.Open();
                if ((int)cmd.ExecuteNonQuery() > 0)
