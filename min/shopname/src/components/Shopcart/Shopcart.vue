...
export default {
  beforeRouteEnter(to, from, next) {
    let total = Object.values(test.goodsObj).reduce(
        (prev, cur) => prev + cur
      );
    next(vm =>{
        vm.$store.dispatch('changeNumByAc', total)
    })
  },
  beforeRouteLeave(to, from, next) {
    ...
    if (result) {
      ...
      this.$store.dispatch('changeNumByAc', total)
  ...
  methods: {
    ...
    del(goods, i) {
      ...
      // 删除test.goodsObj的数据
      this.$store.dispatch('addNumByAc', 0-goods.buyNum)
    },
    substract(goods) {
      goods.buyNum--;
      // test.goodsObj[goods.id]--;
      this.$store.dispatch('addNumByAc', -1)
    },
    add(goods) {
      console.log("加了");
      goods.buyNum++;
